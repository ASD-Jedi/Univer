...
private:
	...
	static LRESULT CALLBACK UsbWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
		switch (uMsg) {
		case WM_CREATE:
			CreateWindow(TEXT("Static"), tempUsb.volumeLabel, WS_CHILD | WS_VISIBLE, 0, 0, WIDTH_NAME, HEIGHT_USB/3, hWnd, (HMENU)50, NULL, NULL);
		...
		case WM_RBUTTONUP:
			...
			for (int i = 0; i < drawnDevices.size(); ++i) {
				if (drawnDevices[i].hWnd == hWnd) {
					remover.eject(drawnDevices[i].deviceLetter);
...
public:
	...
	HWND printDevice(UsbDevice* device, int num) {
		...
		DrawnDevice drawnDevice = { hNewDevice, device->getDevice(num).volumeLetter[0],device->getDevice(num).devInfoData, x, y, num };
