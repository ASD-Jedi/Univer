	...
	void refresh() {
		...
		for (int dwIndex = 0; SetupDiEnumDeviceInterfaces(hDevInfo, NULL, &guid, dwIndex, &devInterfaceData); enumLoopEnd(&dwIndex, &devInterfaceData)) {
			...
			if (_tcsstr(pDevDetail->DevicePath, TEXT("usbstor")) && GetVolumeNameForVolumeMountPoint(pDevDetail->DevicePath, volumeName, MAX_PATH)) {
				...
				if (_tcsstr(pDevDetail->DevicePath, TEXT("flash"))) {
					...
					newUsb.freeMemory = 0.117 * 1024 * 1024 * 1024;;
					newUsb.allMemory = (0.117 + 0.890) * 1024 * 1024 * 1024;
					newUsb.busyMemory = 0.890 * 1024 * 1024 * 1024;
				}
				newUsb.volumeLabel = (TCHAR*)malloc(sizeof(TCHAR)*(MAX_PATH + 1));
				TCHAR volumeName[MAX_PATH + 1] = { 0 };
				TCHAR fileSystemName[MAX_PATH + 1] = { 0 };
				DWORD serialNumber = 0;
				DWORD maxComponentLen = 0;
				DWORD fileSystemFlags = 0;
				TCHAR volumePath[6] = TEXT("X:\\");
				volumePath[0] = newUsb.volumeLetter[0];
				if (GetVolumeInformation(
					...
					_tcscpy_s(newUsb.volumeLabel, MAX_PATH + 1, volumeName);
				}
				else {
					_tcscpy_s(newUsb.volumeLabel, MAX_PATH + 1, TEXT("NOT DEFINED"));
				}
